...
export class ChartsComponent implements OnInit, AfterViewInit, OnDestroy {
  @ViewChild('commissionTimeline', { static: false })
  commissionTimelineEl!: ElementRef;
    ...
    this.deviceService.GetMyDevices(deviceUrl).subscribe({
      next: (res: any) => {
        this.devices = (res.devices ?? res ?? []).map((d: any) => ({
          ...
          commissioningYear: d.commissioningDate
            ? new Date(d.commissioningDate).getFullYear()
            : 0,
  ...
  private renderCommissionTimeline() {
    ...
    const withYear = this.devices.filter(
      (d: any) => d.commissioningYear > 2000,
    );
    if (!withYear.length) return;

    // Group by year then device type
    const types = Array.from(
      new Set(withYear.map((d: any) => d.deviceTypeCode)),
    ).sort();
    const byYear = rollup(
      withYear,
      (v: any) => v.length,
      (d: any) => d.commissioningYear,
      (d: any) => d.deviceTypeCode,
    );
  ...
  private renderCapacityBar() {
    ...
    const grouped = rollup(
      this.devices,
      (v: any) => sum(v, (d: any) => d.capacity),
      (d: any) => d.countryCode,
    );
    const data = Array.from(grouped, ([key, value]) => ({ key, value })).sort(
      (a, b) => b.value - a.value,
    );
    ...
    svg
      ...
      .style('opacity', 0)
      .text((d: any) =>
        d.value >= 1000 ? `${(d.value / 1000).toFixed(1)}MW` : `${d.value}kW`,
      )
      .transition()
      .duration(400)
      .delay((_d: any, i: any) => 400 + i * 60)
      .style('opacity', 1);
  }
  ...
  private renderTopDevices() {
    ...
    const x = scaleLinear()
      .domain([0, max(data, (d: any) => d.capacity)!])
      .nice()
      .range([0, width]);
    const y = scaleBand()
      .domain(data.map((d: any) => d.siteName))
      .range([0, height])
      .padding(0.15);
    ...
    svg
      ...
      .style('fill', '#334155')
      .text((d: any) =>
        d.siteName.length > 16
          ? d.siteName.slice(0, 15) + '\u2026'
          : d.siteName,
      );
  ...
  private renderReadsArea(reads: any[], _device: DeviceRow) {
    ...
    const x = scaleTime()
      .domain(extent(parsed, (d: any) => d.date) as [Date, Date])
      .range([0, width]);
    const y = scaleLinear()
      .domain([0, max(parsed, (d: any) => d.value)!])
      .nice()
      .range([height, 0]);
    ...
    svg
      ...
      .call(
        axisBottom(x)
          .ticks(6)
          .tickFormat((d: any) => timeFormat('%b %d')(d as Date)),
      )
      .call((g: any) => g.select('.domain').attr('stroke', '#e2e8f0'))
      .selectAll('text')
      .style('font-size', '10px')
