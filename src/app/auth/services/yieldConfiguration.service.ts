...
export class YieldConfigurationService {
  ...
  {
    return this.httpClient.get<YieldConfig>(environment.API_URL+'yield/config')
  }
  getyieldInfoById(id:number):Observable<YieldConfig>
  {
    return this.httpClient.get<YieldConfig>(environment.API_URL+'yield/config/'+id)
  }
  addYield(data:any):Observable<YieldConfig>
  {
    return this.httpClient.post<any>(environment.API_URL+'yield/config', data)
